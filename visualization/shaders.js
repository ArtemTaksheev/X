...
X.shaders = function() {
  ...
  //t2 += '     textureSum = texture1;\n';
  //t2 += '     textureSum = vec4(1, .5, 1, 1);\n';
  ...
  t2 += '     textureSum.b = (textureSum.b - _windowLow)/(_windowHigh - _windowLow);\n';
   //t2 += '     textureSum = vec4((textureSum.g - _minrange)/fac,1);\n';
  // map volume scalars to a linear color gradient
  //t2 += '     textureSum = textureSum.g * vec4(volumeScalarMaxColor,1) + (1.0 - textureSum.g) * vec4(volumeScalarMinColor,1);\n';
  t2 += '   }\n';
